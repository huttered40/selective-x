...
void write_kernel_header(std::ofstream& _stream_, bool print_key = true){
  if (print_key){
   ...
   _stream_ << std::left << std::setw(mode_1_width) << "param6";
  }
   _stream_ << std::left << std::setw(mode_1_width) << "skel_count";
   _stream_ << std::left << std::setw(mode_1_width) << "total_exec_time";
   _stream_ << std::left << std::setw(mode_1_width) << "num_k_schedules";
   _stream_ << std::left << std::setw(mode_1_width) << "num_k_samples";
...
/*
      ...
      auto& skel_key_list = skeletonize::active_kernel_keys;
*/
      if (!it.second.is_active) continue;
      int skel_count = -1;
/*
      if (skeletonize::model_container.find(it.first) != skeletonize::model_container.end()){
        skel_count = skel_kernel_list[skeletonize::model_container[it.first].val_index];
      }
*/
      stream_kernel << std::left << std::setw(mode_1_width) << clear_counter + (kk_count*1. / kk_total);
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.key_index].tag;
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.key_index].dim_sizes[0];
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.key_index].dim_sizes[1];
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.key_index].dim_strides[0];
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.key_index].dim_strides[1];
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.key_index].msg_size;
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.key_index].partner_offset;
      stream_kernel << std::left << std::setw(mode_1_width) << skel_count;
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.val_index].total_exec_time;
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.val_index].total_local_exec_time;
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.val_index].num_schedules;
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.val_index].num_local_schedules;
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.val_index].num_scheduled_units;
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.val_index].num_local_scheduled_units;
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.val_index].M1;
      stream_kernel << std::left << std::setw(mode_1_width) << models[it.second.val_index].M2;
      stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_estimate(it.second);
      stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_std_dev(it.second);
      stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_std_error(it.first,it.second);
      stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_confidence_interval(it.first,it.second);
      stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_confidence_interval(it.first,it.second)/accelerate::get_estimate(it.second);
/* TODO: Fix later
      if (comm_kernel_ref_map.find(it.first) != comm_kernel_ref_map.end()){
        auto& ref_p = comm_kernel_ref_map[it.first];
        stream_kernel << std::left << std::setw(mode_1_width) << ref_p.total_exec_time;
        stream_kernel << std::left << std::setw(mode_1_width) << ref_p.total_local_exec_time;
        stream_kernel << std::left << std::setw(mode_1_width) << ref_p.num_schedules;
        stream_kernel << std::left << std::setw(mode_1_width) << ref_p.num_local_schedules;
        stream_kernel << std::left << std::setw(mode_1_width) << ref_p.num_scheduled_units;
        stream_kernel << std::left << std::setw(mode_1_width) << ref_p.num_local_scheduled_units;
        stream_kernel << std::left << std::setw(mode_1_width) << ref_p.M1;
        stream_kernel << std::left << std::setw(mode_1_width) << ref_p.M2;
        stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_estimate(ref_p);
        stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_std_dev(ref_p);
        stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_std_error(it.first,ref_p);
        stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_confidence_interval(it.first,ref_p);
        stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_confidence_interval(it.first,ref_p)/accelerate::get_estimate(ref_p);
    }  
*/
...
/* TODO: Below was used to save kernels for debugging
    if (kernel_execution_control_mode==2){
      for (auto& it : model_container){
        ...
        write_kernel_header(stream_kernel,false);
        ...
        for (auto& iitt : comm_kernel_list[it.first]){
          ...
          stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_estimate(iitt);
          stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_std_dev(iitt);
          stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_std_error(it.first,iitt);
          stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_confidence_interval(it.first,iitt);
          stream_kernel << std::left << std::setw(mode_1_width) << accelerate::get_confidence_interval(it.first,iitt)/accelerate::get_estimate(iitt);
    ...
    for (auto& it : comm_kernel_map){
      ...
      if (world_rank==0) {
        ...
        stream << "\tScheduledTime - " << kernel_list[it.second.val_index].total_exec_time
               ...
               << std::endl;
        stream << "\t\tEstimate - " << accelerate::get_estimate(it.second)
	       << ", DecompTime - " << decomp_time
               << ", StdDev - " << accelerate::get_std_dev(it.second)
               << ", StdError - " << accelerate::get_std_error(it.first,it.second)
               << ", 95% confidence interval len - " << accelerate::get_confidence_interval(it.first,it.second)
               << ", Stopping criterion - " << accelerate::get_confidence_interval(it.first,it.second)/accelerate::get_estimate(it.second)
               << std::endl;
      }
...
void record::write_file(int variantID, int print_mode, float overhead_time){
  ...
  if (world_rank==0){
    if (is_first_iter){
     ...
     write_kernel_header(stream_kernel);
